...
export default async function DishPage({ params }: DishPageProps) {
  ...
  const relatedDishes = await getRelatedDishes(
    dish.category?.id || dish.category?.slug,
    dish.id,
    3,
  );
            ...
            <div>
              ...
              <dl className="mt-5 grid grid-cols-2 gap-3 text-sm">
                ...
                <div className="border border-orange-100 bg-white p-3">
                  ...
                  <dd className="mt-1 font-bold text-stone-950">
                    {dish.restaurant?.name || "Gợi ý bất kỳ"}
                  </dd>
                </div>
                <div className="border border-orange-100 bg-white p-3">
                  <dt className="text-stone-500">Khu vực</dt>
                  <dd className="mt-1 font-bold text-stone-950">
                    {dish.restaurant?.province || "Gần bạn"}
                  </dd>
                </div>
              </dl>
              <div className="mt-5 flex flex-wrap gap-3">
                <Link
                  href="/"
                  className="inline-flex min-h-11 items-center justify-center bg-orange-600 px-5 font-bold text-white hover:bg-orange-700"
                >
                  Random món khác
                </Link>
              </div>
